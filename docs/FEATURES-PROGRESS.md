...
## Unified branch
`claude/emprendehub-unified` merges the **EmprendeHub** advanced base (multi-niche:
academy, nichos, retos, marketplace, teach + `lib/niches`) with all the feature
modules below. Module seed data lives in `lib/data/modules-seed.ts` (decoupled
from the EmprendeHub core `seed.ts`); module queries are appended to
`lib/data/queries.ts`. Build + 320 tests green. `main` stays TradeHub-only.
